...
fn canonical_uri(url: &url::Url) -> String {
    ...
    if path.is_empty() {
        ...
        // `Url::path()` is the serialized wire path. AWS canonicalization URI-encodes every
        // non-unreserved byte again, including the `%` that begins an existing wire escape.
...
mod tests {
    ...
    #[test]
    fn matches_aws_sdk_vector_for_single_encoded_wire_path() {
        ...
        assert_eq!(
            canonical_uri(&url),
            "/model/arn%253Aaws%253Abedrock%253Aus-east-1%253A123456789012%253Aapplication-inference-profile%252Ftest-profile/converse-stream"
        );
        // Fixed external vector generated by botocore 1.43.45 SigV4Auth, the signer used by
        // AWS SDK for Python. It independently applies the AWS double-encoding rule here.
        assert_eq!(
            signed.authorization,
            "AWS4-HMAC-SHA256 Credential=AKIATEST/20250101/us-east-1/bedrock/aws4_request, SignedHeaders=content-type;host;x-amz-content-sha256;x-amz-date;x-amz-security-token, Signature=cb06c104875b9bfbd40471ef6ccfa4dfd482174febb988a6707e40852e5305fe"
        );
